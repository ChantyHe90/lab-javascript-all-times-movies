...
function dramaMovieRate(array) {
    //first seperate the elements that have the value "Drama"
    let dramaEl = array.filter(el => el.genre.includes('Drama'))
    if (dramaEl.length === 0) {
        return undefinded
    }
    let dramaAvg = ratesAverage(dramaEL)
}
dramaMovieRate(movies)
